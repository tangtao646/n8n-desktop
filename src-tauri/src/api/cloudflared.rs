...
static VERSION_REGEX: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"version (\d+\.\d+\.\d+)").expect("版本正则表达式编译失败")
});
...
fn extract_binary_from_tar_gz(archive_path: &Path, destination_path: &Path) -> Result<(), String> {
        ...
        .map_err(|error| format!("读取压缩包条目失败: {}", error))?
    {
        let mut entry = entry_result
            .map_err(|error| format!("处理压缩包条目失败: {}", error))?;
        ...
        if entry_path.file_name().and_then(|name| name.to_str()) == Some("cloudflared") {
            let mut output_file = File::create(destination_path)
                .map_err(|error| format!("创建目标文件 '{}' 失败: {}", destination_path.display(), error))?;
            
            copy(&mut entry, &mut output_file)
                .map_err(|error| format!("提取文件到 '{}' 失败: {}", destination_path.display(), error))?;
...
fn find_cloudflared_in_resource_dir<R: Runtime>(app: &AppHandle<R>) -> Result<Option<String>, String> {
...
pub async fn get_or_download_cloudflared<R: Runtime>(
    ...
    let temp_download_path = download_dir.join(
        if config.is_archive { TEMP_ARCHIVE_FILENAME } else { TEMP_BINARY_FILENAME }
    );
...
fn prepare_download_directory<R: Runtime>(app: &AppHandle<R>) -> Result<PathBuf, String> {
    ...
    if !download_dir.exists() {
        std::fs::create_dir_all(&download_dir)
            .map_err(|error| format!("创建下载目录 '{}' 失败: {}", download_dir.display(), error))?;
    }
    ...
    if is_archive {
        ...
        if final_path.exists() {
            std::fs::remove_file(final_path)
                .map_err(|error| format!("删除现有文件 '{}' 失败: {}", final_path.display(), error))?;
        }
...
fn set_file_permissions(binary_path: &Path) -> Result<(), String> {
    #[cfg(unix)]
    {
        ...
        // 设置可执行权限
        std::fs::set_permissions(binary_path, std::fs::Permissions::from_mode(UNIX_EXECUTABLE_PERMISSIONS))
...
fn extract_cloudflared_version(binary_path: &str) -> Result<Option<String>, String> {
    ...
    let version = VERSION_REGEX.captures(&version_output)
...
fn check_cache_info<R: Runtime>(app: &AppHandle<R>) -> Result<(bool, Option<i64>), String> {
    ...
    let cache_age_days = chrono::Utc::now().signed_duration_since(downloaded_at).num_days();
