...
fn find_windows_node_binary(runtime_dir: &PathBuf) -> PathBuf {
    ...
    search_node_binary(runtime_dir, "node.exe")
...
fn find_unix_node_binary(runtime_dir: &PathBuf) -> PathBuf {
    ...
    search_node_binary(runtime_dir, "bin/node")
        .unwrap_or(direct_path)
}
...
fn build_n8n_command(config: &N8nStartConfig) -> Result<Command, String> {
    let user_data_str = config.user_data_dir.to_str()
        .ok_or("用户数据目录路径包含无效字符".to_string())?;
    
    let mut command = Command::new(&config.node_path);
    
    command.arg(&config.n8n_binary)
        .arg("start")
...
mod tests {
    ...
    fn test_is_node_binary_file() {
        ...
        let node_exe_path = PathBuf::from("C:\\Program Files\\nodejs\\node.exe");
        let other_path = PathBuf::from("/usr/bin/python");
