...
pub async fn download_n8n<R: Runtime>(window: Window<R>, url: String, dest: PathBuf) -> Result<(), String> {
    ...
    while let Some(chunk) = stream.next().await {
        ...
        if total > 0 {
            ...
            if progress - last_emit_progress >= 0.5 || last_emit_time.elapsed() >= Duration::from_millis(150) {
                let _ = window.emit("download-progress", Progress { progress });
    ...
    // 8. 完成
    let _ = window.emit("download-progress", Progress { progress: 100.0 });
    Ok(())
}
